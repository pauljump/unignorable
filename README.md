...
5. Event-level 311 records feed a versioned latent-state model. Distinct report days, direct agency observations, imperfect not-found checks, temporary actions, and evidence age update an estimated presence probability. Evidence relaxes toward uncertainty with a 10-day half-life; duplicate same-day reports are heavily discounted. A closed ticket and the generic “condition was corrected” disposition never mean durable removal.
...
- `POST /api/condition-observations` — proximity-checked, deduplicated encampment field observation used for model calibration
...
- A 311 report is not proof that a condition is present now. Even the encampment estimate is probabilistic and cannot establish exact real-time tent presence without a fresh field observation.
