...
2. The daily local NYC 311 mirror first groups encampment reports at four-decimal coordinate variants, then consolidates variants within 20 meters around fixed evidence-weighted anchors. This produces bounded reported-location envelopes instead of pretending each address geocode is a distinct physical instance. Anchors do not move while neighbors are assigned, so a chain of nearby points cannot merge an entire block. Each feature exposes its source-coordinate count, aliases, maximum offset, and a resulting 35–65 meter uncertainty. Homeless Person Assistance requests are not presented as tent evidence. Other civic categories remain compact recurring clusters.
