...
The September 5 record slice tightens the outcome gate without modifying either model: Held needs reviewed presence before the clear claim, reviewed absence on two distinct later days, and at least one reviewed absence at or after the quiet-window endpoint. A quiet public record is only a clear-claim proxy, not an observed improvement. Pending checks cannot advance Checked. The pilot coordinator must separately establish observer independence and maintain the historical outcome log.
