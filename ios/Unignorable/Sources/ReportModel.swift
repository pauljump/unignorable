...
final class ReportModel: ObservableObject {
    @Published var issues: [ReportIssue] = []
    @Published var enabledTypes: Set<String> = Set(ReportModel.types)
    @Published var position: MapCameraPosition = .region(ReportModel.initialRegion)
    @Published var visibleRegion = ReportModel.initialRegion
    ...
    var markers: [ReportMarker] {
        let latRadius = visibleRegion.span.latitudeDelta * 0.65
        let lngRadius = visibleRegion.span.longitudeDelta * 0.65
        let visible = issues.filter {
            $0.status == "active" && enabledTypes.contains($0.type)
                && abs($0.lat - visibleRegion.center.latitude) <= latRadius
                && abs($0.lng - visibleRegion.center.longitude) <= lngRadius
        }
        if visibleRegion.span.latitudeDelta < 0.022 || visible.count <= 180 {
        ...
        let cell = visibleRegion.span.latitudeDelta > 0.18 ? 0.026
            : visibleRegion.span.latitudeDelta > 0.09 ? 0.016
            : visibleRegion.span.latitudeDelta > 0.045 ? 0.009 : 0.004
        struct Aggregate {
            var weightedLat = 0.0
            var weightedLng = 0.0
            var weight = 0.0
    ...
    private func nearestFirst(_ left: ReportIssue, _ right: ReportIssue) -> Bool {
        let l = pow(left.lat - visibleRegion.center.latitude, 2) + pow(left.lng - visibleRegion.center.longitude, 2)
        let r = pow(right.lat - visibleRegion.center.latitude, 2) + pow(right.lng - visibleRegion.center.longitude, 2)
        return l < r
    }
}
